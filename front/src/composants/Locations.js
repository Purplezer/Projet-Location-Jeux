...
export default function Locations() {
  // Fonction Locations qui retourne le rendu de la page des locations
  ...
  useEffect(() => {
    // Fonction useEffect qui s'active à chaque fois que le composant est monté
    const recup = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3001/utilisateurs/${userID}`
        ); // Requête GET pour récupérer les données utilisateursde la BDD

        if (response.data.length > 0) {
          const locationsResponse = await axios.get(
            `http://localhost:3001/locations/user/${userID}`
          ); // Requête GET pour récupérer les données de location de l'utilisateur de la BDD
          console.log(locationsResponse);
          setLocations(locationsResponse.data);
        } else {
          // Si l'utilisateur n'est pas trouvé dans la BDD
  ...
  const renderLocations = () => {
    // Fonction renderLocations qui permet d'afficher les locations de l'utilisateur
    if (locations.length === 0) {
      // Si l'utilisateur n'a pas de location
      return (
        <div className="locationsvide">
          <p>Aucune location enregistrée.</p>
        </div>
      );
    } else {
      // Si l'utilisateur a des locations
      return (
        // Retourne le rendu des locations
        <ul>
  ...
  const LocationDetails = ({ location, user }) => {
    // Fonction LocationDetails qui retourne le rendu des détails d'une location
    ...
    useEffect(() => {
      ...
      const fetchJeu = async () => {
        try {
          const response = await axios.get(
            `http://localhost:3001/jeux/${location.id_jeu}`
          ); // Requête GET pour récupérer les données du jeu de la BDD
          console.log(response);
          if (response.data.length > 0) {
            // Si le jeu est trouvé dans la BDD
    ...
    const handleEditNote = () => {
      // Fonction handleEditNote qui permet de modifier la note d'une location
      setIsEditingNote(true);
    };

    const handleSaveNote = async () => {
      // Fonction handleSaveNote qui permet d'enregistrer la note d'une location
      try {
        await axios.put(
          `http://localhost:3001/locations/${location.id_location}`,
          {
            // Requête PUT pour mettre à jour la note de la location dans la BDD
            note: newNote,
    ...
    const handleCancelEditNote = () => {
      // Fonction handleCancelEditNote qui permet d'annuler la modification de la note d'une location
      setIsEditingNote(false);
      setNewNote(location.note);
    };

    const handleEditCommentaire = () => {
      // Fonction handleEditCommentaire qui permet de modifier le commentaire d'une location
      setIsEditingCommentaire(true);
    };

    const handleSaveCommentaire = async () => {
      // Fonction handleSaveCommentaire qui permet d'enregistrer le commentaire d'une location
      try {
        await axios.put(
          `http://localhost:3001/locations/${location.id_location}`,
          {
            // Requête PUT pour mettre à jour le commentaire de la location dans la BDD
    ...
    const handleCancelEditCommentaire = () => {
      // Fonction handleCancelEditCommentaire qui permet d'annuler la modification du commentaire d'une location
      setIsEditingCommentaire(false);
      setNewCommentaire(location.commentaire);
    };

    const handleAddNote = () => {
      // Fonction handleAddNote qui permet d'ajouter une note à une location
      setIsEditingNote(true);
    };

    const handleAddCommentaire = () => {
      // Fonction handleAddCommentaire qui permet d'ajouter un commentaire à une location
      setIsEditingCommentaire(true);
    };

    return (
      // Retourne le rendu des détails d'une location
      <div className="locations">
        <div className="locationstitre">
          <h2>Locations</h2>
        </div>
        <div className="locationscontenu">
          <fieldset>
            ...
            {isEditingCommentaire ? (
              ...
              <>
                ...
                <button onClick={handleAddCommentaire}>
  ...
  return (
    <div className="locations">
      ...
      <div className="locationscontenu">{renderLocations()}</div>
    </div>
  );
}
