...
const handleValiderLocation = async (jeu) => {
  ...
  const prixTotal = calculerPrixTotal(jeu.prix, calculerNombreJours(dateLocation, dateRetour));
  ...
  const handleSupprimerLocation = (jeu) => {
        ...
        {panier.length > 0 ? (
          panier.map((jeu, index) => (
            <div className="jeux" key={index}>
              <p>Nom : {jeu.nom_jeu}</p>
              <span className="tailleimagepanier">
              ...
              <p>
                ...
                {calculerPrixTotal(
                  jeu.prix,
                  calculerNombreJours(dateLocation, dateRetour)
                )}{" "}
                €
              </p>
              <button onClick={() => handleValiderLocation(jeu)}>
                Valider
              </button>
              <button onClick={() => handleSupprimerLocation(jeu)}>
                Supprimer
              </button>
            </div>
          ))
